...
IngotsStorage = $4001D327; // Chest in the house in which to throw the chopped. 

HomeRuneBook = $400B601E; // Runabuki ID srnukoy in the house. 
HomeRuneIndex = 0; // position of the runes to the house in runabuka. 0 - the first fleece. 
RuneBookShift = 50; // 75 - chiva, 50 magic. 
HomeX = 1022; // Coordinate X in front of the chest in the house. 
HomeY = 685; // Coordinate in front of the chest in the house. 

Arm = 62; // Amount of physical armor without protect.
Protection = $1F3B; // Scroll type protect. 

Axe = $4006CDC2; // ID of the ax you are hacking. 
...
begin
  ...
  if FindNotoriety(Killers[q],i)>0 then begin
    Result:=True;
    AddToSystemJournal('There was a pk - ' + GetName(FindItem));
    AddToSystemJournal('Runebook # '+IntToStr(CurrentBook+1));
    AddToSystemJournal('Rune # '+IntToStr(CurrentRune+1));
    FindDistance:=2;
    Exit;
...
begin
  ...
  While IsGump do CloseSimpleGump(GetGumpsCount-1);
  cTime:=Now;
  RuneBooks:= [$400B6010]; // Enter the runabuk ID with the runes to the trees here. 
